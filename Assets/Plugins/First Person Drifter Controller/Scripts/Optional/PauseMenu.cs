...
public class PauseMenu : MonoBehaviour
{
	...
	void Start ()
	{
		print ("loadedLevel is " + Application.loadedLevel);
		paused = false;
	...
	void OnGUI()
	{
		if( paused )
		{
			...
			else if( mouseSettings)
			{
				...
				// invertY button
				if( GUI.Button(new Rect(Screen.width/2 - buttonWidth/2, Screen.height/2 - buttonHeight/0.8f, buttonWidth, buttonHeight), invertYText, pauseMenuStyle) )
				{
					InvertY();
				}
				
				// SFX Label
				GUI.Label(new Rect( Screen.width/2 - buttonWidth/2 - sliderWidth, Screen.height/2 - buttonHeight/1.5f, buttonWidth, buttonHeight), "Field of view [" + (int)fovSlider + "]", pauseMenuStyle);
				...
				GUI.Label(new Rect( Screen.width/2 - buttonWidth/2 - sliderWidth, Screen.height/2 + buttonHeight/1.5f, buttonWidth, buttonHeight), "Volume [" + (int)volumeSlider + "]", pauseMenuStyle);
				
				// FOV SLIDER
				GUI.skin.horizontalSliderThumb = scrollButtonStyle;
				fovSlider = GUI.HorizontalSlider (new Rect (Screen.width/2, Screen.height/2 - buttonHeight/1.5f + sliderHeight/2, sliderWidth, sliderHeight), fovSlider, minFOV, maxFOV, scrollBarStyle, scrollButtonStyle);
				SetFOV();
				...
				//volume
				GUI.skin.horizontalSliderThumb = scrollButtonStyle;
				volumeSlider = GUI.HorizontalSlider (new Rect (Screen.width/2, Screen.height/2 + buttonHeight/1.5f + sliderHeight/2, sliderWidth, sliderHeight), volumeSlider, 0, 3, scrollBarStyle, scrollButtonStyle);
				SetVolume();
				
				if( GUI.Button(new Rect(Screen.width/2 - buttonWidth/2, Screen.height/2 + buttonHeight/0.8f, buttonWidth, buttonHeight), "Reset to default", pauseMenuStyle) )
				{
					ResetToDefault();
				}
			}
			else if( quitDialog )
			{
				DrawPauseMenuBG();
				// Are you sure label
				GUI.Label(new Rect( Screen.width/2 - buttonWidth/2, Screen.height/2 - buttonHeight, buttonWidth, buttonHeight), "Progress saved. Quit?", pauseMenuStyle);
